...
while ($step -le 5) {
    ...
    switch ($step) {
        1 {
            Write-Host "`n[Step 1/5] Select Environment" -ForegroundColor Yellow
            $envs = Get-CondaEnvs
        ...
        2 {
            Write-Host "`n[Step 2/5] Project Workspace" -ForegroundColor Yellow
        ...
        3 {
            Write-Host "`n[Step 3/5] Dataset Configuration" -ForegroundColor Yellow
        ...
        4 {
            Write-Host "`n[Step 4/5] Model Task & Architecture" -ForegroundColor Yellow
        ...
        5 {
            Write-Host "`n[Step 5/5] Select Hyperparameters" -ForegroundColor Yellow
            ...
            Write-Host "- Enter interger (e.g. 16, 32): Fixed Batch Size" -ForegroundColor DarkGray
...
Clear-Host
Write-Host "==========================================" -ForegroundColor Cyan
Write-Host "         [Ready to Train!]                " -ForegroundColor Cyan
Write-Host "==========================================" -ForegroundColor Cyan
...
if ($config.task -eq "auto") {
    $trainCmd = "yolo mode=train data=`"$($config.yamlPath)`" model=`"$($config.modelFile)`" epochs=$($config.epochs) imgsz=$($config.imgsz) batch=$($config.batch) project=`"runs`" name=`"train_result`""
} else {
    $trainCmd = "yolo task=$($config.task) mode=train data=`"$($config.yamlPath)`" model=`"$($config.modelFile)`" epochs=$($config.epochs) imgsz=$($config.imgsz) batch=$($config.batch) project=`"runs`" name=`"train_result`""
}
